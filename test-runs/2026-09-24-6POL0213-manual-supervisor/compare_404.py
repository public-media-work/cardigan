...
print("#404  word_count (is_srt=True):", u404.calculate_transcript_metrics(SRT, long_form_threshold_minutes=15, is_srt=True)["word_count"])

split = split_interior_speaker_changes(SRT)
ch_main = main_split(split, is_srt=True, config=cfg_main["routing"]["chunking"])
print("main  chunking:", "single call" if ch_main is None else f"{len(ch_main)} chunks", cfg_main["routing"]["chunking"])
